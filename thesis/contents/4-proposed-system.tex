...
At first, only Wang et al.'s SMILE Twitter emotion dataset was found, containing a total of 3,085 tweets classified as: anger, disgust, happy, no-code, not-relevant, sad and surprise \cite{wang2016smile}. By simply taking into account the number of tweets offered in this dataset as small, and probably not enough, to perform \acrshort{dl} based classification. Figure \ref{fig:smile_dataset_distribution} show the distribution of tweets related to each annotated class. By analyzing the number of tweets related to anger the amount of tweets are reduced to 57. Moreover, the tweets labeled as not-relevant and no-code, classes that contains no value to the purpose of the research, represents around 58\% of the whole dataset. However, even if not being enough, the remaining 42\% would be stored to posteriorly merged to other datasets.

\begin{figure}[!htp]
  \center
  \includegraphics[width=0.6\textwidth]{figures/SMILE_dataset_distribution}
...
\begin{figure}[!htp]
  ...
  \caption{Wang 2012's test and train\_2\_1 datasets merge distribution.}
  \label{fig:wang_2012_dataset_distribution}
\end{figure}

To prevent \acrshort{dl} overfitting issue, use dropout layer, validation data, obtain more data, under-sample data.
