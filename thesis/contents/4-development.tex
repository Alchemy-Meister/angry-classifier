...
Started implementing the Convolution Neural Network (CNN) by using Keras python module for Deep Learning.
...
On the home server running the same test on a GPGPU configuration in conjunction with CUDA 8 and CUDA Deep Neural Network (cuDNN) v5, it took 2 hours 9 minutes to perform the same task. Proving that GPGPU can speed up 52.59\% the process.
...
The language model estimates the probability of a words by counting the number of appearance. The tests runs on a dictionary that uses Project Gutenberg’s free eBooks, Wiktionary most frequents words and British National Corpus (Model side ~6Mb.)
...
To improve the model first I searched for some n-gram database. Google has a huge that includes 5-grams (~88Gb), However the access to this resource costs 150 USD for non-members. Therefore, I downloaded all the English Ebooks from Project Gutenberg as for 2014 (~42Gb). After solving encoding errors in some file names I start the eBook preprocessing.
...
I found a new model, from Fréderic Goding that has been trained with 400 millions of tweets and uses 400 features.
