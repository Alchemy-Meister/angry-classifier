...
def main(argv):
    ...
    for ndx, (classifier_name, classifier_dict) in enumerate( \
        ...
        #TODO Remove softmax layer of the model.
        model = remove_layers(model, 1)

        for layer in model.layers:
            model.get_layer(name=layer.name).name = layer.name + '_' + str(ndx)

        models.append(model)

    agregated = merge([x.layers[-1].output for x in models], mode='concat')
    agregated = Dense(4, activation='softmax', name='agregated_softmax')(agregated)

    aggregated_model = Model(input=[x.input for x in models], output=[agregated])

    aggregated_model.compile(loss='categorical_crossentropy', optimizer='adam', \
        metrics=['accuracy', 'mse', 'mae'])

    aggregated_model.summary()

    #TODO exit model modification loop
